...
删除媒体行为说明：
- 列表页和详情页都提供删除入口，提交后会直接删除媒体记录。
- 当前版本采用直接物理删除：若该媒体文件没有被其他记录引用，会同时删除宿主机上的原始文件。
- 为兼容历史数据，如果还有其他记录复用同一个 `storage_path`，本次只删除当前记录，不删除共享物理文件。
- 删除操作需要已登录会话，并通过会话 CSRF token 校验。
- 逻辑删除、限时删除和自动物理删除暂未实现，后续再迭代。
...
- 已支持上传岛屿增强版：多文件选择、拖拽高亮、图片本地预览/视频占位、客户端类型/大小校验、逐文件上传状态、总体进度、失败重试、上传成功后列表即时插入、最近上传结果反馈。
